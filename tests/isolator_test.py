...
def assert_isolate(extractor_result, isolated_data):
    ...
    assert len(extractor_result.columns) == len(isolated_data_column_names), "unexpected column length: expected {}, got {}({}) ".format(
        len(extractor_result.columns), isolated_data_column_names, len(isolated_data_column_names)
    )
