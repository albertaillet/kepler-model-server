    ...
    def __init__(self, model_path, model_name, output_type, model_file, features, fe_files, component_init=False):
        ...
        if self.comp_type:
            ...
            for comp, model_metadata in model_info.items():
                model = ScikitModel(model_path, self.name, self.output_type.name, model_metadata["model_file"], model_metadata["features"], model_metadata["fe_files"], component_init=True)
